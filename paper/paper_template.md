...
Generated research build: {{BUILT_AT}}  
Source lexicon: `{{LEXICON_SPEC}}`  
Repository revision: `{{SOURCE_REVISION}}`

## Abstract

This paper tests a bounded version of Linguistical Symmetry Theory: that explicit opposition can provide a useful structural spine for a lexical graph while synonymy supplies neighborhoods around opposed poles. The claim is tested against the core 2025 edition of Open English WordNet, using senses rather than spellings as the principal lexical unit and preserving source provenance throughout the build. The resulting graph contains {{WORDS}} lexical entries, {{LEMMAS}} distinct normalized lemmas, {{FORMS}} word forms, {{SENSES}} senses, {{SYNSETS}} synsets, and {{RELATIONS}} typed relation records. It contains {{OPPOSITION_PAIRS}} unique explicit opposition pairs after deduplicating direction while retaining relation type and endpoint kind. {{INCIDENT_SENSES}} senses ({{INCIDENT_SENSES_PCT}}) and {{INCIDENT_LEMMAS}} lemmas ({{INCIDENT_LEMMAS_PCT}}) participate in at least one direct sense-level opposition. Among sense-level opposition pairs, {{BOTH_MULTI_COUNT}} ({{BOTH_MULTI_PCT}}) have multi-lemma synonym neighborhoods at both poles. These findings support opposition as a real but limited organizing structure in the selected lexical source. They do not support the stronger claim that most English vocabulary is antonymous, that information-theoretic redundancy is synonymy, or that the measured graph supplies a complete semantics for natural-language programming.
...
The normalized graph contains {{WORDS}} lexical entries, {{LEMMAS}} distinct normalized lemmas, {{FORMS}} forms, {{SENSES}} senses, {{SYNSETS}} synsets, and {{RELATIONS}} typed relation records. {{MULTI_SYNSETS}} synsets contain more than one distinct lemma. {{SENSE_SYNONYM_COUNT}} senses ({{SENSE_SYNONYM_PCT}}) and {{LEMMA_SYNONYM_COUNT}} lemmas ({{LEMMA_SYNONYM_PCT}}) have at least one co-synonym under the operational definition used here.
...
The source contains {{OPPOSITION_DIRECTED}} directed opposition relation records. After deduplicating direction while preserving endpoint kind and relation type, the graph contains {{OPPOSITION_PAIRS}} unique opposition pairs: {{SENSE_OPPOSITION_PAIRS}} sense-level pairs and {{SYNSET_OPPOSITION_PAIRS}} synset-level pairs.

{{INCIDENT_SENSES}} senses ({{INCIDENT_SENSES_PCT}}) and {{INCIDENT_LEMMAS}} distinct lemmas ({{INCIDENT_LEMMAS_PCT}}) participate in a direct sense-level opposition. {{RECIPROCAL_PAIRS}} unique pairs ({{RECIPROCAL_PCT}}) are encoded in both directions. The undirected opposition graph divides into {{COMPONENTS}} connected components, and its largest component contains {{LARGEST_COMPONENT}} endpoints.
...
Among sense-level opposition pairs, {{ONE_MULTI_COUNT}} ({{ONE_MULTI_PCT}}) have a multi-lemma synonym neighborhood at one or both poles. {{BOTH_MULTI_COUNT}} ({{BOTH_MULTI_PCT}}) have such neighborhoods at both poles. {{EQUAL_NEIGHBOR_COUNT}} pairs ({{EQUAL_NEIGHBOR_PCT}}) have equal neighborhood sizes, and the median size-balance ratio is {{MEDIAN_BALANCE}}.
